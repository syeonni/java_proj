    		...
    		try {
    			while (true) {
    				// 읽은 메시지의 종류에 따라 각각 할일이 정해져 있음
           	 		...
           	 		else if (type == ChatMessage.MsgType.GAME_START){ //게임 시작시 자기 자신의 캐릭터와 자기자신의 이름 모두에게 보냄
           	 			broadcastMessage(new ChatMessage(ChatMessage.MsgType.GAME_START, message.userCheck,message.character,message.startSender));
           	 		}
           	 		else if (type == ChatMessage.MsgType.GAME_DRAW){ //그릴 것의 좌표 정보를 모두에게 보내줌
           	 			broadcastMessage(new ChatMessage(ChatMessage.MsgType.GAME_DRAW, message.x,message.y,message.height,message.width,message.color));
           	 		}
           	 		else if (type == ChatMessage.MsgType.GAME_CURRECT){ //정답을 맞추면 모두에게 알림
           	 			broadcastMessage(new ChatMessage(ChatMessage.MsgType.GAME_CURRECT, message.Currecter));
           	 		}
           	 		else if (type == ChatMessage.MsgType.GAME_TIME){ //출제자의 시간을 모두 공유
           	 			broadcastMessage(new ChatMessage(ChatMessage.MsgType.GAME_TIME, message.time));
           	 		}
           	 		else {
           	 			// 정체가 확인되지 않는 이상한 메시지?
